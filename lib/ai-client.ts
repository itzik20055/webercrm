...
const ExtractedLeadSchema = z.object({
  customerName: z
    ...
    .nullable()
    .describe(
      "Customer's name if explicitly stated in the conversation. From self-introduction ('שלום, מדבר משה כהן') or salesperson addressing them ('אז יוסי, מה דעתך'). Return null if no name was spoken, or if the chat is anonymized and only [NAME] appears."
    ),
...
12. **שם הלקוח (customerName)**: זה השדה הכי חשוב לחלץ מתוך תמלולי שיחות טלפון. עבוד שיטתית:
    - אם רואה רק [NAME] בכל ההודעות (השיחה כבר מאונונמת) → החזר null. אל תנסה.
    - **חפש בכל מקום בשיחה**, לא רק בהתחלה:
      a. **הצגה עצמית של הלקוח**: "שלום, מדבר משה", "כאן יוסף וייס", "Hi, this is David", "מי מדבר? — חיים"
      b. **המוכר פונה ללקוח בשם**: "אז משה, מה דעתך?", "תראה יוסי...", "Sarah, let me check"
      c. **שם נמסר במהלך השיחה**: "תרשום שלמה כהן", "השם המלא הוא דוד פרידמן", "אני אביגדור"
      d. **שמות בני משפחה שמספרים על עצמם**: "אני ובעלי רחל..." (אז שם הלקוח רחל אם זו אישה), "אני וחיים..." (חיים = בן הזוג)
    - **אל תבלבל**:
